`timescale 1ns / 1ps
// =============================================================================
// cnn_top.sv  —  Full system integration top module
//
// Integrates (using the ACTUAL port lists of the supplied modules):
//   conv_block (x CONV_NUMB)   — contains its own internal kernel_memory
//   relu       (x CONV_NUMB)   — combinational activation after each conv stage
//   max_pooling_block (x CONV_NUMB)
//   flat
//   fully_connected_layer (x FLAT_NUMB)
//   runtime_weight_loader (x CONV_NUMB + FLAT_NUMB) — NEW, was missing
//   status_register                                  — NEW, was missing
//
// The reference file CNN.sv hard-wired weights at elaboration time via
// CNN.svh and used a conv_block / fully_connected_layer port list
// (weights_mem_in_*) that does NOT match the actual RTL you supplied
// (conv_block.sv / fully_connected_layer.sv use weight_load_en /
// weight_addr / weight_data[, weight_sel]).  This top module instead wires
// the *real* modules together and adds a single external runtime
// configuration bus, demultiplexed to one runtime_weight_loader per
// learnable layer, plus a status_register used as a pipeline-progress
// monitor (the datapath itself is free-running / no-backpressure, exactly
// as documented in the original CNN.sv header).
// -----------------------------------------------------------------------------
...
always @(posedge clk) begin
    $display("%0t relu0=%0b pool0=%0b relu1=%0b pool1=%0b fc0=%0b fc1=%0b",
             $time,
             relu0_done_mon,
             pool0_done_mon,
             relu1_done_mon,
...
always @(posedge clk) begin
    if(pool_eop[0] || pool_eop[1])
        $display("%0t pool_eop0=%0b pool_eop1=%0b pool0_done_mon=%0b pool1_done_mon=%0b",
                 $time,
                 pool_eop[0],
                 pool_eop[1],
                 pool0_done_mon,
...
always @(posedge clk) begin
    if(pool1_done_mon)
        $display("%0t POOL1_DONE_MON PULSE",$time);
end
    ...
    always @(posedge clk) begin
    if(pool1_done_mon)
        $display("%0t POOL1_DONE_MON state=%0d",
                 $time,
                 inst_status_register.state);
end
   always @(posedge clk) begin
    if(start)
        $display("%0t TOP_START=1",$time);
end

always @(posedge clk) begin
    if(conv_valid[0]) $display("CONV0_VALID");
    if(conv_valid[1]) $display("CONV1_VALID");

    if(pool_valid[0]) $display("POOL0_VALID");
    if(pool_valid[1]) $display("POOL1_VALID");

    if(fc_valid[0]) $display("FC0_VALID");
    if(fc_valid[1]) $display("FC1_VALID");
end
always @(posedge clk) begin
    if(conv_eop[0])
        $display("%0t CONV0_EOP",$time);

    if(conv_eop[CONV_NUMB-1])
        $display("%0t CONV_LAST_EOP",$time);

    if(pool_eop[0])
        $display("%0t POOL0_EOP",$time);

    if(pool_eop[CONV_NUMB-1])
        $display("%0t POOL_LAST_EOP",$time);

    if(fc_eop[0])
        $display("%0t FC0_EOP",$time);

    if(fc_eop[FLAT_NUMB-1])
        $display("%0t FC_LAST_EOP",$time);
        
end
always @(posedge clk) begin
    $display("%0t fc_eop0=%0b fc_eop1=%0b",
             $time,
             fc_eop[0],
             fc_eop[FLAT_NUMB-1]);
end
always @(posedge clk) begin
    ...
    if(pool_eop[1])
        $display("%0t POOL1_EOP",$time);
end
always @(posedge clk) begin
    $display("%0t state=%0d relu0=%0b pool0=%0b relu1=%0b pool1=%0b fc0=%0b fc1=%0b",
              $time,
              inst_status_register.state,
              relu0_done_mon,
              pool0_done_mon,
...
always @(posedge clk) begin
if(conv_eop[0])
    $display("conv_valid0=%b conv_eop0=%b",
             conv_valid[0],
             conv_eop[0]);
   end          
   always @(posedge clk) begin
    if(stream_valid)
        $display("TOP_STREAM data=%0d", stream_data);
end
always @(posedge clk) begin
    if(fc_valid[0])
        $display("%0t FC0_VALID data=%0d",
                 $time,
                 $signed(fc_data[0]));
...
always @(posedge clk) begin
    if(inst_status_register.state >= 5)
        $display("%0t FC_STATE=%0d fc0_done=%0b fc1_done=%0b",
                 $time,
                 inst_status_register.state,
                 fc0_done_mon,
                 fc1_done_mon);
...
always @(posedge clk) begin
    if(fc_eop[0] || fc_eop[1])
        $display("%0t FC_EOP0=%0b FC_EOP1=%0b STATE=%0d",
                 $time,
                 fc_eop[0],
                 fc_eop[1],
                 inst_status_register.state);
...
always @(posedge clk) begin
    if(fc_eop[0])
        $display("%0t ===== FC0_EOP ===== STATE=%0d",
                 $time,
                 inst_status_register.state);

    if(fc_eop[1])
        $display("%0t ===== FC1_EOP ===== STATE=%0d",
                 $time,
                 inst_status_register.state);
end
always @(posedge clk) begin
    if(fc0_done_mon)
        $display("%0t FC0_DONE", $time);

    if(fc1_done_mon)
        $display("%0t FC1_DONE", $time);
end
always @(posedge clk) begin
    if(conv_eop[0] || conv_eop[1] ||
       pool_eop[0] || pool_eop[1]) begin

        $display("%0t conv_eop0=%0b conv_eop1=%0b pool_eop0=%0b pool_eop1=%0b",
                 $time,
                 conv_eop[0],
                 conv_eop[1],
                 pool_eop[0],
...
always @(posedge clk) begin
    if(conv_eop[0])
        $display("%0t CONV0_EOP valid=%0b sop=%0b eop=%0b",
                 $time,
                 conv_valid[0],
                 conv_sop[0],
                 conv_eop[0]);
...
always @(posedge clk) begin
    if(pool_valid[1])
        $display("%0t POOL1_VALID eop=%0b",
                 $time,
                 pool_eop[1]);
end 
always @(posedge clk) begin
    if(conv_valid[1])
        $display("%0t CONV1_VALID eop=%0b",
                 $time,
                 conv_eop[1]);
end
always @(posedge clk) begin
    if(conv_eop[0])
        $display("%0t CONV0_EOP state=%0d",
                 $time,
                 inst_status_register.state);
end 
always @(posedge clk) begin
    if(pool_eop[1])
        $display("%0t POOL1_EOP state=%0d",
                 $time,
                 inst_status_register.state);
end
always @(posedge clk) begin
    if(inst_status_register.state==4 || pool_eop[1])
        $display("%0t STATE=%0d pool_eop0=%0b pool_eop1=%0b",
                 $time,
                 inst_status_register.state,
                 pool_eop[0],
                 pool_eop[1]);
